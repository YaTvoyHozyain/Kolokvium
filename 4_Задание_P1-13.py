...
# P-8 - Умножение многочленов (пока с ссылками такими)
# Морик, 1308
...
def MUL_PP_P(m1, C1, m2, C2):
    ...
    m = MUL_Pxk_P(m2, C2, m1)[0]  # степень рез-рующего полиному - рез-т умножения 1го полинома на степ. переменной 2го
...
def null_polynomial(C, m):  # проверка на то, является ли полином нулевым
...
def GCF_PP_P(m1, C1, m2, C2):
    ...
    for i in range(len(coef1)):
        r = MOD_PP_P(DEG_P_N(coef), coef, deg2, coef2)  # остаток от coef при делении на меньший степенью полином
