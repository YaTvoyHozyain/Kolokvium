...
# m – массив цифр степени многочлена, массив C коэффициентов, k - массив цифр степени переменной, на которую умножается
def MUL_Pxk_P(m, C, k):
    C = [0] * ARR_TO_INT(k) + C  # в начало добавляется столько нулей, насколько увеличивается в степень
...
def FAC_P_Q(m, arr1):
    ...
    j = 0
    m1 = ARR_TO_INT(m)
    # Переделываем в положительное натуральное числитель
    # тк знаменатель и так натуральный
...
# C1 - коэффициенты первого многочлена, m1 - массив цифр старшей степени первого многочлена,
# C2 - коэффициенты второго многочлена, m2 - массив цифр старшей степени второго многочлена
...
def MUL_PP_P(m1, C1, m2, C2):
    ...
    m, _ = MUL_Pxk_P(m2, C2, m1)[0]  # степень рез-рующего полинома - рез-т умножения 1го полинома на степень x 2го
    C = [0 for _ in range(ARR_TO_INT(ADD_1N_N(m)))]  # количество коэфф-тов = степень полученного многочлена + 1

    for i in range(len(С1)):  # чтобы второй полином перемножился на все коэффициенты первого полинома
        m0, C0 = MUL_PQ_P(m2, C2, C1[i])  # умножаем второй полином на i-тый коэффициент первого полинома
        _, C = ADD_PP_P(m0, C0, i, C)  # к рез-рующему полиному добавляется полученный в кач-ве домножения,
        # старшей степени соответствует номер нынешней итерации

    return m, C  # возвращается старшая степень полученного многочлена и массив его коэффициентов
...
# P-11 Морик
# НОД многочленов
# C1 - массив коэффициентов первого многочлена, m1 - массив цифр старшей степени первого многочлена,
# C2 - массив коэффициентов второго многочлена, m2 - массив цифр старшей степени второго многочлена
...
def GCF_PP_P(m1, C1, m2, C2):
    ...
    # если делится изначально без остатка, то в кач-ве НОД возвращаем делитель
    if NZER_N_B(MOD_PP_P(deg1, coef1, deg2, coef2)) == 1:
        return deg2, coef2

    while True:
        # поиск предпоследнего элемента в делении полиномов в столбик
        time_deg1, time_coef1 = deg1, coef1[:len(coef2)]
        k = ARR_TO_INT(ADD_1N_N(DEG_P_N(coef2)))  # для передвижения по коэффициентам делимого

        # доходим до нужного элемента посредством деления в столбик
        while MOD_PP_P(deg1, coef1, deg2, coef2) != MOD_PP_P(time_deg1, time_coef1, deg2, coef2):
            time_deg1, time_coef1 = MOD_PP_P(time_deg1, time_coef1, deg2, tcoef2)
            time_coef1 += coef1[k]
            k = ADD_1N_N(k)

        # делимое теперь - вычитаемое из предпоследнего элемента (относительно остатка)
        deg1, coef1 = SUB_PP_P(time_deg1, time_deg1, *MOD_PP_P(time_deg1, time_coef1, deg2, coef2))
        # делитель - сам остаток
        deg2, coef2 = MOD_PP_P(time_deg1, time_coef1, deg2, coef2)

        # если остаток от деления ноль, значит, нашли НОД
        if NZER_N_B(MOD_PP_P(time_deg, time_coef, deg2, coef2)) == 1:
            return deg2, coef2
...
def DER_P_P(m, C):
    ...
    for x in range(m):
        ...
        #заполнение пустого массива цифрами из числа, проверка на дробность
        d = 0
        for i in str(C[x]):
            if i == "." and b1 != 1:
                n = d
                ...
                C_new.append(int(i))
            d += 1
        n1 = len(C_new)#количество цифр в массиве
        #вычисление степени, на которую нужно умножить
        M = []
        X = []
        for i in str(m):
            M.append(int(i))
        ...
        A = SUB_NN_N(M, X)
        b2 = 0#знак степени
        n2 = len(A)#количество цифр в степени
        b3,n3,C[x] = MUL_ZZ_Z(b1, n1, C_new, b2, n2, A)#умножение коэффициентов на соответствующую им степень
        #добавление знака ".", если число было дробным и "-", если было отрицательным
        if n and n > 0:
            C[x] = C[x][:n+(n3-n1)] + ["."] + C[x][n+(n3-n1):]
        if b1 == 1:
            C[x] = ["-"] + C[x]
        #удаление знака "." и 0 на конце, если результат - недробное число
        if C[x][len(C[x])-1] == 0 and C[x][len(C[x])-2] == ".":
            C[x].pop(len(C[x])-2)
            C[x].pop(len(C[x])-1)
        #приведение числа из массива цифр в строку
        str1 = ""
        ...
        for i in C[x]:
            if i == ".":
                n = -2
        #приведение строки к числу в соответствии с его типом
        if n == -2:
            C[x] = float(str1)
        else:
            C[x] = int(str1)
