...
def RED_Q_Q(num, numb, denom):
    # проверяем чилитель на 0 {Z-2, POZ_Z_D} и {N-2, NZER_N_B}
    if NZER_N_B([POZ_Z_D(num, numb)]) != 1:
...
def INT_Q_B(num, numb, denom):
    # проверяем целый числитель на 0 {Z-2, POZ_Z_D} и {N-2, NZER_N_B}
    if NZER_N_B([POZ_Z_D(num, numb)]) != 1:
...
# выход: массив цифр числителя, знак дроби, массив цифр знаменателя
def TRANS_Z_Q(A, b):
    # если целое число не 0 {Z-2, POZ_Z_D} и {N-2, NZER_N_B}
    if NZER_N_B([POZ_Z_D(A, b)]) != 1:
...
def TRANS_Q_Z(num, numb, denom):
    ...
    num, numb, denom = RED_Q_Q(num, numb, denom)
    ...
    if COM_NN_D([result1], [1]) == 0:
        # если дробь(числитель) не 0, вернуть числитель и знак {Z-2, POZ_Z_D} и {N-2, NZER_N_B}
        if NZER_N_B([POZ_Z_D(num, numb)]) != 1:
            return num, numb
        else:
...
def ADD_QQ_Q(num1, numb1, denom1, num2, numb2, denom2):
    ...
    num1 = MUL_ZZ_Z(num1, numb1, koef1[0], koef1[1])
    num2 = MUL_ZZ_Z(num2, numb2, koef2[0], koef2[1])
    # складываем числители {Z-6, ADD_ZZ_Z}
    numres = ADD_ZZ_Z(num1[0], num1[1], num2[0], num2[1])
    # если полученный числитель не 0 {Z-2, POZ_Z_D} и {N-2, NZER_N_B}
    if NZER_N_B([POZ_Z_D(numres[0], numres[1])]) != 1:
        # сократим полученную дробь {Q-1, RED_Q_Q}
        qres = RED_Q_Q(numres[0], numres[1], lcm)
        return qres[0], qres[1], qres[2]
    else:
        # иначе вернуть нулевую дробь [0], 0, [1] (1,тк знаменатель не мб 0) {Q-3, TRANS_Z_Q}
        qzero = TRANS_Z_Q(numres[0], numres[1])
        return qzero[0], qzero[1], qzero[2]
...
def MUL_QQ_Q(num1, num1b, denom1, num2, num2b, denom2):
    # если числители не равны 0 {Z-2, POZ_Z_D} и {N-2, NZER_N_B}
    if NZER_N_B([POZ_Z_D(num1, num1b)]) != 1 and NZER_N_B([POZ_Z_D(num2, num2b)]) != 1:
